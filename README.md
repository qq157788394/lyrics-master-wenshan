...
├── .workbuddy/skills/lyrics-master-wenshan/   ← Skill 本体
│   ├── SKILL.md
│   ├── references/       方法论速查 / 韵脚表 / 课程分析 / 古词表
│   └── templates/        六个阶段产物模板
├── tools/                Skill Creator 打包脚本
...
1. 到 [Releases](https://github.com/qq157788394/lyrics-master-wenshan/releases) 下载最新 `lyrics-master-wenshan.skill`
2. 将 `.skill` 文件解压（本质是 zip）到你的 `~/.workbuddy/skills/` 或项目内的 `.workbuddy/skills/`
3. 重新加载 Agent 即可触发
...
cp -r lyrics-master-wenshan/.workbuddy/skills/lyrics-master-wenshan ~/.workbuddy/skills/
```
...
# 产物：dist/lyrics-master-wenshan.skill
```

CI 会在推送 `v*` tag 时自动打包并发布到 GitHub Release。
