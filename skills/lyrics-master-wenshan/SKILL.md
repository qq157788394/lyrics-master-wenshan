...
主 Agent 在全部流程中必须遵守以下三条铁律：

**1. 硬关卡：** 每个 Phase 都有对应的硬关卡。硬关卡未通过，不得进入下一环节。主 Agent 在进入下一 Phase 前，必须逐项确认当前 Phase 的硬关卡是否全部满足。具体硬关卡见各 Phase 的说明。

**2. 主 Agent 只调度，不创作：** 主 Agent 的职责是流程调度——收集需求、调子 Agent、写文件、管理反馈队列。主 Agent 绝不产出任何歌词内容、不提供任何创作建议、不对子 Agent 的产出做任何润色或修改。

**3. 子 Agent 方案原样交付：** 子 Agent 返回的方案，主 Agent 必须原封不动展示给用户。不做总结、不提炼要点、不添加任何自己的评论。用户看到的内容 = 子 Agent 输出的内容。
...
6. **建立待办清单**：主 Agent 使用 TodoWrite 为自己创建本次创作的待办清单，任务项为 Phase 1→Phase 2→Phase 3→Phase 4→Phase 5→Phase 6。后续每完成一个 Phase 即标记完成

### Phase 1：聊需求（主 Agent 直营）

主 Agent **一次性**向用户抛出以下 7 个问题，等待用户回答。如果用户回答不完整，再针对缺失项逐一追问：
...
3. 若修改：主 Agent 追问「还有其他修改意见吗？」，尽量一次性收集所有反馈。确认用户没有其他意见后，汇总写入 `feedback-queue.md`，`MODE=modify` 调子 Agent 一次批量修改
...
- **先词后曲模式**：主 Agent 先问用户——选歌曲结构（方文山三大基本曲式：①A1-A2-B1-B2 两段主歌+两段副歌 ②A-B1-B2 主歌简化快速进副歌 ③B-A-B1-B2 副歌前置先抛Hook），再问两个独立问题：是否需要导歌（Pre-Chorus）？是否需要桥段（C）？
